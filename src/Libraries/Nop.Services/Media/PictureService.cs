    ...
    public partial class PictureService : IPictureService
    {
        ...
        protected virtual string GetThumbUrl(string thumbFileName, string storeLocation = null)
        {
            storeLocation = !String.IsNullOrEmpty(storeLocation)
                                    ? storeLocation
                                    : GetStoreLocation();//_webHelper.GetStoreLocation();
        ...
        #region TestWebHelper
        private string GetStoreLocation()
        {
            bool useSsl = IsCurrentConnectionSecured();

            if (_mediaSettings.LogImagePathGeneration)
                _logger.Information(string.Format("Log for picture: useSsl {0}", useSsl.ToString()));

            return GetStoreLocation(useSsl);
        }

        private  bool IsCurrentConnectionSecured()
        {
            bool useSsl = false;
        ...
        private string ServerVariables(string name)
        {
            ...
            try
            {
                ...
                {
                    if (_mediaSettings.LogImagePathGeneration)
                        _logger.Information(string.Format("Log for picture: Request not available"));
        ...
        private string GetStoreLocation(bool useSsl)
        {
            ...
            if (_mediaSettings.LogImagePathGeneration)
                _logger.Information(string.Format("Log for picture: result Host is {0}", result.ToString()));
            ...
            { 
                ...
                if (_mediaSettings.LogImagePathGeneration)
                    _logger.Information(string.Format("Log for picture: result Request is {0}", result.ToString()));
            }
        ...
        private string GetStoreHost(bool useSsl)
        {
            var result = "";
            var httpHost = ServerVariables("HTTP_HOST");

            if (_mediaSettings.LogImagePathGeneration)
                _logger.Information(string.Format("Log for picture: httpHost {0}", httpHost.ToString()));
            ...
            if (DataSettingsHelper.DatabaseIsInstalled())
            {
                ...
                if (_mediaSettings.LogImagePathGeneration)
                    _logger.Information("Log for picture: Database is installed");
                ...
                if (String.IsNullOrWhiteSpace(httpHost))
                {
                    ...
                    if (_mediaSettings.LogImagePathGeneration)
                        _logger.Information(string.Format("Log for picture: result at httpHost empty {0}", result.ToString()));
                ...
                if (useSsl)
                {
                    ...
                    if (_mediaSettings.LogImagePathGeneration)
                        _logger.Information(string.Format("Log for picture: Use SSL is true result {0} and currentStore.SecureUrl {1} ", result.ToString(), currentStore.SecureUrl));
                }
                else
                {
                    if (currentStore.SslEnabled && !String.IsNullOrWhiteSpace(currentStore.SecureUrl))
                    {
                        ...
                        result = currentStore.Url;
                        if (_mediaSettings.LogImagePathGeneration)
                            _logger.Information(string.Format("Log for picture: Use SSL is false and currentStore.Url is {0}", result.ToString()));
                    }
                }
                #endregion
            }
            else
            {
                ...
                if (useSsl)
                {
                    ...
                    if (_mediaSettings.LogImagePathGeneration)
                        _logger.Information(string.Format("Log for picture: Database is not installed and result is {0}", result.ToString()));
                }
                #endregion
            }
            ...
            PictureType defaultPictureType = PictureType.Entity)
        {
            ...
            if(_mediaSettings.LogImagePathGeneration)
                _logger.Information(string.Format("Log for picture: ID {0} thumbFileName {1} thumbFileName {2} storeLocation", picture.Id, thumbFileName, thumbFilePath, storeLocation));

            url = GetThumbUrl(thumbFileName, storeLocation);

            if (_mediaSettings.LogImagePathGeneration)
                _logger.Information(string.Format("Log for picture: url {0}", url));
