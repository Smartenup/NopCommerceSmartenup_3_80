...
namespace Nop.Plugin.Shipping.ByWeight
{
    public class ByWeightShippingComputationMethod : BasePlugin, IShippingRateComputationMethod
    {
        ...
        public GetShippingOptionResponse GetShippingOptions(GetShippingOptionProductRequest getShippingOptionProductRequest)
        {
            ...
            var storeId = getShippingOptionProductRequest.StoreId;
            if (storeId == 0)
                storeId = _storeContext.CurrentStore.Id;
            int countryId = getShippingOptionProductRequest.ShippingAddress.CountryId.HasValue ? getShippingOptionProductRequest.ShippingAddress.CountryId.Value : 0;
            int stateProvinceId = getShippingOptionProductRequest.ShippingAddress.StateProvinceId.HasValue ? getShippingOptionProductRequest.ShippingAddress.StateProvinceId.Value : 0;
            int warehouseId = getShippingOptionProductRequest.WarehouseFrom != null ? getShippingOptionProductRequest.WarehouseFrom.Id : 0;
            string zip = getShippingOptionProductRequest.ShippingAddress.ZipPostalCode;
            decimal subTotal = decimal.Zero;

            if (!getShippingOptionProductRequest.Product.IsFreeShipping)
                //TODO we should use getShippingOptionRequest.Items.GetQuantity() method to get subtotal
                subTotal += getShippingOptionProductRequest.Product.Price;
            
            decimal weight = getShippingOptionProductRequest.Product.Weight;
